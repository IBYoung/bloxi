...
impl Bloxi {
    ...
    fn proof_of_work(&self) -> BlockProof {
        ...
        while !Bloxi::valid_proof(self.last_block(), &current_proof) {
            current_proof.0 += 1;
        }
        current_proof
    }
    ...
    pub fn get_block(&self, height: usize) -> &Block {
        self.chain
            .get(height)
            .expect("the block is not match any records")
    }
    ...
    pub fn reconcile(&self) -> impl Future<Item = Bloxi, Error = ()> {
            ...
            .map(|node| {
                ...
                let f_or_err = builder
                    ...
                    .map(|req| {
                        ...
                        let boxed: Box<Future<Item = Result<Chain, ()>, Error = ()>> = Box::new(f);
                        boxed
                    });
        ...
        if blocks.len() > 0 {
            blocks
                ...
                .all(|(current_block, last_block)| {
                    ...
                    let proof_check = Bloxi::valid_proof(last_block, &current_block.proof);
